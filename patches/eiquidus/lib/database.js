...
function find_tx(txid, cb) {
  Tx.findOne({txid: txid}).select('-_id -__v').lean().then((tx) => {
    if (tx) {
      // convert total field from Decimal128 to string
...
function needs_decimal_migration(cb) {
  // check if one field isn't Decimal128 that should be
  const filter = {
    $or: [
      { $expr: { $ne: [ { $type: "$balance" },  "decimal" ] } }
    ]
  };

  Address.findOne(filter, { projection: { _id: 1 } }).then((doc) => {
    return cb(null, !!doc); // true => needs migration
  }).catch((err) => {
    console.log(err);
    return cb(err);
  });
...
function migrate_numeric_fields_to_decimal128(cb) {
  function castToDecimal(field) {
    const v = `$${field}`;
    const t = { $type: v };
    ...
    return {
      $switch: {
        branches: [
          // already decimal
          { case: { $eq: [t, "decimal"] }, then: v },

          // if it's a double, snap to integer, then go long -> decimal
          {
            case: { $eq: [t, "double"] },
            then: {
              $toDecimal: {
          ...
          // ints/longs can go straight to decimal exactly
          { case: { $eq: [t, "int"] }, then: { $toDecimal: v } },
          { case: { $eq: [t, "long"] }, then: { $toDecimal: v } },

          // strings that contain big integers/decimals
          { case: { $eq: [t, "string"] }, then: { $toDecimal: v } },
        ],
        default: {
          // fallback: attempt conversion, keep original on error
          $convert: { input: v, to: "decimal", onError: v, onNull: null }
        }
      }
    };
  }
  ...
  let filter = {
    $or: [
      { $expr: { $ne: [ { $type: "$received" }, "decimal" ] } },
      { $expr: { $ne: [ { $type: "$sent" },     "decimal" ] } },
      { $expr: { $ne: [ { $type: "$balance" },  "decimal" ] } }
    ]
  };
  ...
  let pipeline = [
    {
      $set: {
        received: castToDecimal("received"),
        sent: castToDecimal("sent"),
        balance: castToDecimal("balance")
      }
    }
  ];

  // update the received, sent and balance fields in the address collection
  Address.updateMany(filter, pipeline, { bypassDocumentValidation: true }).then((address_res) => {
    filter = {
      $or: [
        { $expr: { $ne: [ { $type: "$amount" }, "decimal" ] } }
      ]
    };
    ...
    pipeline = [
      {
        $set: {
          amount: castToDecimal("amount")
        }
      }
    ];

    // update the amount field in the addresstxes collection
    AddressTx.updateMany(filter, pipeline, { bypassDocumentValidation: true }).then((addresstx_res) => {
      filter = {
        $or: [
          { $expr: { $ne: [ { $type: "$total" }, "decimal" ] } }
        ]
      };
      ...
      pipeline = [
        {
          $set: {
            total: castToDecimal("total")
          }
        }
      ];

      // update the total field in the txes collection
      Tx.updateMany(filter, pipeline, { bypassDocumentValidation: true }).then((tx_res) => {
        filter = {
          $or: [
            { $expr: { $ne: [ { $type: "$supply" }, "decimal" ] } },
            { $expr: { $ne: [ { $type: "$last_price" }, "decimal" ] } },
            { $expr: { $ne: [ { $type: "$last_usd_price" }, "decimal" ] } }
          ]
        };
        ...
        pipeline = [
          {
            $set: {
              supply: castToDecimal("supply"),
              last_price: castToDecimal("last_price"),
              last_usd_price: castToDecimal("last_usd_price")
            }
          }
        ];

        // update the supply field in the stats collection
        Stats.updateMany(filter, pipeline, { bypassDocumentValidation: true }).then((stats_res) => {
          return cb(null);
        }).catch((err) => {
          console.log(err);
          return cb(err);
...
function check_number_vs_decimal(cb) {
  needs_decimal_migration(function(err, needed) {
    if (err)
      return cb(err);
    else if (!needed)
      return cb();

    migrate_numeric_fields_to_decimal128(cb);
  });
}
...
function formatNetworkChartValue(val, roundDecimals, lbl) {
  let max = 20;
  ...
  return lib.format_decimal_string(new Decimal((val || 0).toString()), { minFractionDigits: 0, maxFractionDigits: max }) + (lbl == 'Hashrate' ? ' ' + getNetHashUnits() : '');
}
...
module.exports = {
  ...
  connect: function(database, cb) {
    ...
    if (database == null) {
      if (settings.dbsettings.user && settings.dbsettings.password) {
        database = 'mongodb://' + encodeURIComponent(settings.dbsettings.user) + ':' + encodeURIComponent(settings.dbsettings.password) + '@' + settings.dbsettings.address + ':' + settings.dbsettings.port + '/' + settings.dbsettings.database;
      } else {
        database = 'mongodb://' + settings.dbsettings.address + ':' + settings.dbsettings.port + '/' + settings.dbsettings.database;
      }
    }
  ...
  update_richlist: function(list, cb) {
    ...
    if (list == 'received') {
      ...
      if (
        settings.richlist_page.burned_coins.addresses == null ||
        settings.richlist_page.burned_coins.addresses.length == 0 ||
        !settings.richlist_page.burned_coins.include_burned_coins_in_distribution
      ) {
        ...
        // do not omit burned addresses from database query. instead, increase the limit of returned addresses
        // and manually remove each burned address that made it into the rich list after recording the burned balance
        Address.aggregate([
          { $sort: { balance: -1 } },
          ...
          let burned_balance = new Decimal('0');
          ...
          addresses.forEach(function (address) {
            ...
            if (burn_addresses.findIndex(p => p.toLowerCase() == address.a_id.toLowerCase()) > -1) {
              ...
              burned_balance = burned_balance.add(new Decimal(address.balance.toString()));
          ...
          Richlist.updateOne({coin: settings.coin.name}, {
            ...
            burned: [ mongoose.Types.Decimal128.fromString(burned_balance.toString()) ]
          }).then(() => {
            return cb();
          }).catch((err) => {
            console.log(err);
  ...
  get_last_txs: function(start, length, min, internal, cb) {
    this.get_last_txs_ajax(start, length, min, function(txs, count) {
      if (!Array.isArray(txs)) { return cb([], 0); }
      ...
      async.timesSeries(txs.length, function(i, loop) {
        ...
        get_extracted_by_addresses(show_extracted_by, internal, txs[i], function(extracted_by_addresses) {
          if (internal) {
            ...
            row.push(lib.format_decimal_string(new Decimal(txs[i].total.toString()).div(100000000), { minFractionDigits: 2, maxFractionDigits: 8 }));
            ...
            let data_entry = {
              ...
              amount: new Decimal(txs[i].total.toString()).div(100000000).toString(),
  ...
  get_last_txs_ajax: function(start, length, min, cb) {
    ...
    if (min > 0) {
      ...
      Tx.countDocuments({'total': {$gte: min}}).then((count) => {
  ...
  get_address_txs_ajax: function(hash, start, length, cb) {
    AddressTx.countDocuments({a_id: hash}).then((totalCount) => {
      AddressTx.aggregate([
        { $match: { a_id: hash } },
        { $sort: { blockindex: -1 } },
        { $skip: Number(start) },
  ...
  get_distribution: function(richlist, stats, cb) {
    const distribution = {
      supply: new Decimal(stats.supply.toString()),
      t_1_25: {percent: new Decimal('0'), total: new Decimal('0') },
      t_26_50: {percent: new Decimal('0'), total: new Decimal('0') },
      t_51_75: {percent: new Decimal('0'), total: new Decimal('0') },
      t_76_100: {percent: new Decimal('0'), total: new Decimal('0') },
      t_101plus: {percent: new Decimal('0'), total: new Decimal('0') }
    };

    async.timesSeries(richlist.balance.length, function(i, loop) {
      const percentage = new Decimal(richlist.balance[i].balance.toString()).div(100000000).div(stats.supply.toString()).mul(100);

      if ((i + 1) <= 25) {
        distribution.t_1_25.percent = distribution.t_1_25.percent.add(percentage);
        distribution.t_1_25.total = distribution.t_1_25.total.add(new Decimal(richlist.balance[i].balance.toString()).div(100000000));
      }

      if ((i + 1) <= 50 && (i + 1) > 25) {
        distribution.t_26_50.percent = distribution.t_26_50.percent.add(percentage);
        distribution.t_26_50.total = distribution.t_26_50.total.add(new Decimal(richlist.balance[i].balance.toString()).div(100000000));
      }

      if ((i + 1) <= 75 && (i + 1) > 50) {
        distribution.t_51_75.percent = distribution.t_51_75.percent.add(percentage);
        distribution.t_51_75.total = distribution.t_51_75.total.add(new Decimal(richlist.balance[i].balance.toString()).div(100000000));
      }

      if ((i + 1) <= 100 && (i + 1) > 75) {
        distribution.t_76_100.percent = distribution.t_76_100.percent.add(percentage);
        distribution.t_76_100.total = distribution.t_76_100.total.add(new Decimal(richlist.balance[i].balance.toString()).div(100000000));
      }

      loop();
    }, function() {
      distribution.t_101plus.percent = new Decimal('100').sub(distribution.t_76_100.percent).sub(distribution.t_51_75.percent).sub(distribution.t_26_50.percent).sub(distribution.t_1_25.percent).sub((settings.richlist_page.burned_coins.include_burned_coins_in_distribution == true && richlist.burned > 0 ? new Decimal(richlist.burned.toString()).div(100000000).div(stats.supply).mul('100') : new Decimal('0'))).toFixed(2);
      distribution.t_101plus.total = distribution.supply.sub(distribution.t_76_100.total).sub(distribution.t_51_75.total).sub(distribution.t_26_50.total).sub(distribution.t_1_25.total).sub((settings.richlist_page.burned_coins.include_burned_coins_in_distribution == true && richlist.burned > 0 ? new Decimal(richlist.burned.toString()).div(100000000) : new Decimal('0'))).toFixed(8);
      distribution.t_1_25.percent = distribution.t_1_25.percent.toFixed(2);
      distribution.t_1_25.total = distribution.t_1_25.total.toFixed(8);
      distribution.t_26_50.percent = distribution.t_26_50.percent.toFixed(2);
      distribution.t_26_50.total = distribution.t_26_50.total.toFixed(8);
      distribution.t_51_75.percent = distribution.t_51_75.percent.toFixed(2);
      distribution.t_51_75.total = distribution.t_51_75.total.toFixed(8);
      distribution.t_76_100.percent = distribution.t_76_100.percent.toFixed(2);
      distribution.t_76_100.total = distribution.t_76_100.total.toFixed(8);
          ...
          Stats.findOne({coin: coin}).then((stats) => {
            if (stats) {
              Stats.updateOne({coin: coin}, {
                coin: coin,
                count : count,
                supply: mongoose.Types.Decimal128.fromString((supply ? supply : new Decimal('0')).toString()),
                connections: (connections ? connections : 0)
              }).then(() => {
                return cb({
                  coin: coin,
                  count : count,
                  supply: (supply ? supply : new Decimal('0')),
                  connections: (connections ? connections : 0),
  ...
  get_masternode_rewards_totals: function(mnPayees, since, cb) {
    Tx.aggregate([
      { $match: { blockindex: { $gt: Number(since) }, vin: [] } },
      { $unwind: "$vout" },
      { $match: { "vout.addresses": { $in: [mnPayees] } } },
      {
        $group: {
          _id: null,
          total: {
      ...
      return cb(data.length > 0 ? new Decimal(data[0].total.toString()).div(100000000) : new Decimal('0'));
    }).catch((err) => {
      console.log(err);
      return cb(null);
    });
  ...
  update_last_updated_stats: function (coin, param, cb) {
    const allowed = [
      'blockchain_last_updated',
      'reward_last_updated',
      'masternodes_last_updated',
      'network_last_updated',
      'richlist_last_updated',
      'markets_last_updated',
    ];
    const key = allowed.find(k => param && param[k]);

    // check if an update key was found
    if (!key) {
      // invalid option
      return cb(false);
    } else {
  ...
  get_network_chart_data: function(cb) {
    ...
    NetworkHistory.find().sort({blockindex: 1}).lean().exec().then((data) => {
      const roundDecimalsNetHash = settings.shared_pages.page_header.network_charts.nethash_chart.round_decimals || -1;
      const roundDecimalsDifficulty = settings.shared_pages.page_header.network_charts.difficulty_chart.round_decimals || -1;

      data.forEach(function (row) {
        // add fixed values for display
        row['nethashFixed'] = formatNetworkChartValue(row.nethash || 0, roundDecimalsNetHash, 'Hashrate');
        row['difficultyPowFixed'] = formatNetworkChartValue(row.difficulty_pow || 0, roundDecimalsDifficulty, 'Difficulty');
        row['difficultyPosFixed'] = formatNetworkChartValue(row.difficulty_pos || 0, roundDecimalsDifficulty, 'Difficulty');
      });
              ...
              module.exports.check_masternodes(function(masternodes_exists) {
                ...
                module.exports.check_networkhistory(function(networkhistory_exists) {
                  ...
                  module.exports.check_peers(function(peers_exists) {
                    // ensure old number fields are typed as Decimal128
                    check_number_vs_decimal(function(number_err) {
  ...
  get_orphans: function(start, length, cb) {
    ...
    Orphans.countDocuments({}).then((count) => {
