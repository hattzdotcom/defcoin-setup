...
if (lib.is_locked([database]) == false) {
  ...
  if (lib.is_locked(['backup', 'restore', 'delete']) == false) {
    ...
    if (settings.dbsettings.user && settings.dbsettings.password) {
      dbString = 'mongodb://' + encodeURIComponent(settings.dbsettings.user) + ':' + encodeURIComponent(settings.dbsettings.password) + '@' + settings.dbsettings.address;
    } else {
      dbString = 'mongodb://' + settings.dbsettings.address;
    }
