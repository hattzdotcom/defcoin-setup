    ...
    function OnBlockchainSynced(syncedCallback){
        ...
        var checkSynced = function(displayNotSynced){
            ...
            if (options.coin.reward == "POW"){
                gbtParams = [{"capabilities": [ "coinbasetxn", "workid", "coinbase/append" ], "rules": [ "segwit", "mweb" ]}];
            }
    ...
    function DetectCoinData(finishedCallback){
        ...
        _this.daemon.batchCmd(batchRpcCalls, function(error, results){
            ...
            for (var i = 0; i < results.length; i++){
                ...
                if (rpcCall !== 'submitblock' && rpcCall !== 'getnetworkinfo' && (r.error || !r.result)){
            ...
            // Synthesize legacy getinfo from modern RPC responses
            var bcInfo = rpcResults.getblockchaininfo || {};
            var netInfo = rpcResults.getnetworkinfo || {};
            rpcResults.getinfo = {
                testnet: bcInfo.chain === 'test',
                protocolversion: netInfo.protocolversion || 70017,
                connections: netInfo.connections || 0,
                difficulty: bcInfo.difficulty || 0,
                blocks: bcInfo.blocks || 0
            };
